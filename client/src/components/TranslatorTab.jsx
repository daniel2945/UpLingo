  ...
  return (
    <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 animate-in fade-in zoom-in-95 duration-300">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={() => setIsToHebrew(!isToHebrew)}
          className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 px-4 py-2 rounded-xl text-slate-700 font-bold transition"
        >
          {isToHebrew
            ? `${targetLangName} לעברית`
            : `עברית ל-${targetLangName}`}{" "}
      ...
      <textarea
        ...
        className="w-full h-32 p-4 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:border-blue-500 outline-none resize-none text-lg"
        dir="auto"
      />
      <button
        onClick={handleTranslate}
        disabled={isTranslating || !transInput}
        className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-lg transition flex justify-center items-center gap-2 disabled:opacity-50"
      >
      ...
      {transResult && (
        <div className="mt-8 p-6 bg-blue-50 border border-blue-100 rounded-2xl relative">
          <p className="text-xl font-medium text-slate-800" dir="auto">
            {transResult}
          </p>
          {transInput.split(" ").length <= 3 && (
            <button
              ...
              disabled={isAdding}
              className="mt-4 flex items-center gap-2 text-sm font-bold bg-white border border-slate-200 px-4 py-2 rounded-lg text-slate-600 hover:text-blue-600 hover:border-blue-300 transition"
            >
              {isAdding ? (
